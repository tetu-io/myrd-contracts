...
export class TimeUtils {
  ...
  public static async mineAndCheck() {
    ...
    while (true) {
      await ethers.provider.send('evm_mine', []);
      if ((await ethers.provider.getBlockNumber()) > start) {
        break;
      }
  ...
  public static async getBlockTime(multicall: Multicall, block?: number | null): Promise<number> {
    if (block) {
      return Number(await multicall.getCurrentBlockTimestamp({ blockTag: block }));
    } else {
      return Number(await multicall.getCurrentBlockTimestamp());
    }
  }
